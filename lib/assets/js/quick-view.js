(function(it_exchange_quick_view) {
	...
	it_exchange_quick_view(window.jQuery, window, document);
		...
		$(function() {
						...
						$.colorbox({
							...
							onComplete: function() {
								...
								$( '#it-exchange-quick-view-container' ).on( 'submit', '.it-exchange-quick-view-purchase-options form.it-exchange-sw-add-to-cart', function( event ) {
									$( '#it-exchange-quick-view-container' ).html( '<div id="it-exchange-quick-view-processing"><span></span><br/>This will keep loading until we find a way trigger a custom event.</div>' ).addClass( 'add-to-cart-mode' );
								});

								$( document ).on( 'click', '.it-exchange-super-widget .it-exchange-empty-cart', function( event ) {
									$( '#it-exchange-quick-view-container' ).fadeOut(250);
									window.setTimeout( function() { $.colorbox.remove() }, 250 );
								});
			...
			// GLENN: This is where I wanted to an action trigger by the added to cart event.
			//        The event itself is easy to trigger with jQuery. It's just a matter of
			//        adding that to the SW Ajax that handles adding the item to the cart.
			//
			//        $.event.trigger({
			//            type: "itExchangeProductAddedToCart",
			//        });

			$( document ).on( 'itExchangeProductAddedToCart', function( event ) {
				event.preventDefault();	
